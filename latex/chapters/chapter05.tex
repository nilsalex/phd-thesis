...
\begin{equation}
  \delta^a_b = T^an_b + e^a_\alpha \epsilon^\alpha_b = \frac{1}{N}\delta^a_0 n_b - \frac{1}{N} N^a n_b + \delta^a_\alpha \epsilon^\alpha_b,
\end{equation}
a vector field $v$ decomposes as
\begin{equation}
  v(\mathrm dt) = v(n)\quad\text{and}\quad v(\mathrm dx^\alpha) = -\frac{1}{N}N^\alpha v(n) + v(\epsilon^\alpha),
\end{equation}
completely determining the field by virtue of $N$, $N^\alpha$, $v(n)$, and $v(\epsilon^\alpha)$. For the area metric field, we obtain the decomposition
\begin{equation}
