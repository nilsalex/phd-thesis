...
The rules of the tensor calculus we are going to implement can now be stated using the generalized rank of a tensor. Note that some rules already follow from the definition of a generalized rank, but are stated again for completeness.
\begin{enumerate}
...
\end{code}
The \mintinline{haskell}{Tensor} type is a so-called \emph{generalized algebraic datatype}, short GADT, which means that the type of each constructor can be specified explicitly. There are three of such constructors:
\begin{itemize}
...
\end{code}
\begin{code}
  \begin{minted}[frame=single,samepage]{text}
λ> let delta_ac = delta :: Tensor '[ '( 'VSpace "ST" 4, 'ConCov ("a" ':| '[]) ("c" ':| '[]))] Int
  \end{minted}
  \captionof{listing}{Addition of tensors in the interactive repl \emph{ghci}}
  \label{repl_addition}
\end{code}
