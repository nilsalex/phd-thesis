...
\end{equation}
which depends on the wavefront $S$ only via the wave covector $k=-\mathrm dS$. Eq.~\ref{wkb_section} is a linear equation for the amplitudes $a^B$, which is determined by the $m\times m$ matrix $T_{AB}(k)$, called the \emph{principal symbol} of the field equations. If the theory admits solutions with nontrivial amplitudes $a^B$ in the WKB limit, the principal symbol $T_{AB}(k)$ must necessarily be noninjective. By virtue of this condition, the principal polynomial has the important r\^ole to select physically admissible wave covectors in the limit of short wavelengths. As a square matrix is noninjective if and only if its determinant vanishes, admissible wave covectors can equivalently be characterized by a vanishing condition on the determinant of $T_{AB}(k)$.
...
The condition that the kernel of the principal symbol be of dimension $s+1$ or higher is equivalent to imposing that all order-$s$ adjugate matrices
\begin{equation}
  Q^{(A_1\dots A_s)(B_1\dots B_s)}(k) \vcentcolon= \frac{\partial^s \operatorname{det}(T_{AB}(k))}{\partial T_{A_1B_1}(k)\dots T_{A_sB_s}(k)}
\end{equation}
vanish.

The mathematical formulation of the second axiom is concerned with properties of the gravitational theory in the short-wavelength approximation.

Requiring a gravitational theory to be diffeomorphism invariant is not enough in order to guarantee a physically meaningful dynamical evolution of the gravitational field. To ensure this is the objective of the second axiom of covariant constructive gravity: causal compatibility with the dynamics of matter fields. The purpose of this axiom is twofold. First, since we will consider matter theories 
