...
\begin{axiom}[Diffeomorphism invariance \cite{Alex_2020}]
  The dynamical laws that govern gravity are invariant under spacetime diffeomorphisms.
\end{axiom}
\begin{axiom}[Causal compatibility \cite{Alex_2020}]
...
Lagrangian field theory now stipulates that sections $\sigma\in\Gamma(\pi)$ are physical if they are extremals of the action functional. The well-known \emph{Euler-Lagrange equations} from the calculus of variations provide a necessary condition in local coordinates which such sections extremals must satisfy.
...
The intrinsic equivalent to the Euler-Lagrange equations in local coordinates introduces a new object, the \emph{Cartan form}, which plays a central r\^ole in the geometrization of Lagrangian field theory.
\begin{proposition}[\cite{Saunders_1989}]
...
This definition generalizes the local derivation of \eqref{euler_lagrange_local}: the variation $\delta L$ is obtained by lifting $\mathrm d\mathscr L$ to $\pi_{2k}$ and cancelling nonhorizontal terms (over $E$) by adding a derivative, which corresponds to repeated integrations by parts.
...
\end{equation}
where the forms $\psi^A_{p_1\dots p_s} = \mathrm d u^A_{p_1\dots p_s} - u^A_{p_1\dots p_sq} \mathrm dx^q$ span the contact system of $\pi_k$ (see \cite{Saunders_1989}).
...
We wish to encode \eqref{lagrangian_diffeo_invariance} as local conditions on the Lagrangian density. To this end, consider a coordinate representation $\mathscr L = L\mathrm d^nx$. 
\begin{proposition}
  Let $\mathscr L = L_{(x)}\mathrm d^nx$ be a coordinate representation of a diffeomorphism invariant Lagrangian, induced by a coordinate chart $(U,x)$ on $M$. It follows from the invariance condition \eqref{lagrangian_diffeo_invariance} that $L_{(x)}$ is \textbf{diffeomorphism equivariant}, i.e.\ it holds for all $\varphi\in\mathrm{Diff}(M)$ that, over the intersection of $U$ and $\varphi(U)$,
...
Proceeding to derive the infinetismal version of axiom 1, we first need to specify what is meant by \emph{infinitesimal}. As the symmetry group in question is the diffeomorphism group on the base manifold, the infinitesimal equivalent is the corresponding Lie algebra $\Gamma(TM)$ of sections of the tangent bundle over $M$. The Lie bracket is given by the Lie bracket of vector fields. In a given coordinate chart, an element $\xi\in\Gamma(TM)$ defines an infinitesimal diffeomorphism as
...
For the mathematical formulation of the second axiom, we utilize the close relation of the causal structure of field equations to the short-wavelength limit of the theory. \cite{R_tzel_2011,D_ll_2018} First, we restrict to Lagrangians which are \emph{degenerate} in the sense that the Euler-Lagrange equations---although defined on $J^4E$---depend only on second derivatives and lower, i.e.
\begin{equation}
