...
\section{Construction of the third-order area metric Lagrangian}
...
\subsection{Iterative solution strategy for gravitational field equations}
...
\subsection{Solution in Einstein gravity}
...
\begin{equation}
  \Gamma^\alpha_{\hphantom\alpha 00} = -\frac{1}{2}\partial^\alpha h_{00} + \dot h^{\alpha}_{\hphantom\alpha 0} + \mathcal O(h^2),
\end{equation}
...
The integrals that remain to evaluate are
\begin{equation}
  K^{\alpha\beta} = \int\mathrm d^3\vec y\sum_{i=1,2}\rho_{(i)}(\tau,\vec y)\dot\gamma^\alpha_{(i)}\dot\gamma^\beta_{(i)}
\end{equation}
