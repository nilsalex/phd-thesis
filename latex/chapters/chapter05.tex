...
\end{equation}
where $E_0 = -mc^2$ and $E = -\frac{1}{2} m \eta r^2\omega^2$. For the gravity part, we again use \texttt{cadabra} in order to derive the contributions from the transverse traceless $U^{\alpha\beta}$ modes \eqref{area_metric_radiation_massless} to $\frac{\delta L_\text{gravity}}{\delta b^\alpha}$. This yields via a similar calculation as before
\begin{equation}
  \begin{aligned}
