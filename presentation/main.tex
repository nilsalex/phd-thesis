...
\begin{document}
    ...
    \begin{frame}{Space tells matter how to move \ldots}
        \begin{itemize}
            \item Das gesamte Standardmodell der Teilchenphysik (SMTP) ist von metrischer Geometrie bestimmt.
            \item \ldots{} wer oder was bestimmt die Geometrie?
        \end{itemize}
    \end{frame}

    \begin{frame}{\ldots{} Matter tells space how to curve}
        \begin{itemize}
            \item Gravitation ist die dynamische Theorie der Raumzeitmetrik
            \item Einstein-Hilbert-Wirkung
            \[ S = \int \sqrt{-g} R d^4 x \]
            \item Bzw.\ Einsteinsche Feldgleichungen
            \[ G_{ab} = T_{ab} \]
        \end{itemize}
    \end{frame}
    ...
    \begin{frame}{Modifizierte Gravitation}
        \begin{itemize}
            ...
            \begin{itemize}
                \item<1-> Rotationskurven von Galaxien
                \item<1-> Singularitäten (Schwarze Löcher, Urknall)
                \item<1-> Teilchenphysik ??
                \item<1-> \ldots
            \end{itemize}
            \item<2-> Modifikationen oder Erweiterungen unterliegen den Eindeutigkeitsresultaten!
            \begin{itemize}
                \item<2-> Materietheorie bleibt rein metrisch, oder
                \item<2-> \alert{\tikzmarknode{a}{Materietheorie benutzt andere Geometrie, oder}}
                \item<2-> Grundannahmen müssen verworfen werden
            \end{itemize}
        \end{itemize}
        \onslide<3>
        \begin{tikzpicture}[overlay,remember picture]
            \draw<3>[->,mLightBrown,thick,smooth,out=30,in=270] ([xshift=-.4cm,yshift=-1.2cm]a.south) to ([xshift=.3cm,yshift=-3pt]a.south);
            \node<3>[mLightBrown] at ([xshift=-.85cm,yshift=-1.45cm]a.south) {Constructive Gravity};
        \end{tikzpicture}
    \end{frame}
    ...
    \begin{frame}{Kanonische Konstruktive Gravitation}
        \begin{itemize}
            \item Resultat von HKT lässt sich verallgemeinern für beliebige tensorielle Geometrien
            \item Prinzip: Constraint-Algebra der Gravitation muss die Hyperflächendeformationsalgebra implementieren
            \item Entspricht einem unendlichen System von PDEs für die Lagrangedichte
            \item Störungstheorie oder Symmetriereduktion möglich
            \item Kausalität ??
    ...
    \begin{frame}{Exakte Lösung: Einstein}
        \begin{itemize}
            ...
            Dimension $4+10+40+100 = 154$ ($\dim M = 4$)
