...
We will first consider a distribution of test matter and study the effect of a passing gravitational wave, which yields the usual deformations known as geodesic deviation, amended by novel deformation patterns. A more direct effect of the radiation that is emitted by a binary star is its energy loss, which makes the system spin faster. This will serve as second example.
...
\end{equation}
For small deviations (\textbf{or derivatives of deviations?}), the deviation due to purely spatial perturbations $\varphi^{\alpha\beta}$ is integrated as
\begin{equation}
