...
\end{equation}
and the pull-back action on cotangent vectors\footnote{The action is inverted in order to still define a covariant functor.}
\begin{equation}
...
\begin{equation}
  \begin{aligned}
  u^{a_1\dots a_m}_{b_1\dots b_n} &= I^{a_1\dots a_m}_{b_1\dots b_nA} u^A \\
  u^A &= J^{Ab_1\dots b_n}_{a_1\dots a_m} u^{a_1\dots a_m}_{b_1\dots b_n}
  \end{aligned}
\end{equation}

First, it needs to be specified what is meant by \emph{infinitesimal}. As the symmetry group in question is the diffeomorphism group on the base manifold, the infinitesimal equivalent is the corresponding Lie algebra $\Gamma(TM)$ of sections of the tangent bundle over $M$. The Lie bracket is given by the Lie bracket of vector fields. In a given coordinate chart, an element $\xi\in\Gamma(TM)$ defines an infinitesimal diffeomorphism as
...
The Gotay-Marsden coefficients are the defining objects for the PDE version of the invariance of $L$ under infinitesimal diffeomorphisms. This constitutes the central result concerning axiom 1 and shall be proved in the following.
