...
The first step is to realize that the Cartan form for a diffeomorphism invariant Lagrangian is diffeomorphism invariant.
...
\end{proof}
Using the infinitesimal diffeomorphism invariance condition $\mathcal L_{\xi_{J^{2k-1}E}} \Theta_L = 0$, the first Noether theorem follows as a direct consequence.
...
\begin{theorem}[stress-energy-momentum tensor]
  Let $\Theta_L$ be the diffeomorphism invariant Cartan form corresponding to a diffeomorphism invariant Lagrangian of second derivative order ($k=2$). For any local section $\sigma$ of the underlying bundle, there exists a unique $(1,1)$-tensor density $\boldsymbol{\mathcal T}(\sigma)$ on the base manifold $M$ such that for all vector fields $\xi$ on $M$ and embedded hypersurfaces $i_\Sigma \colon \Sigma \rightarrow M$
  \begin{equation}\label{sem_definition}
    \int_\Sigma i_\Sigma^\ast\boldsymbol{j}(\sigma) = \int_\Sigma \boldsymbol{\mathcal T}^{\,n}_{\,m}(\sigma)\xi^m \omega_n.
  \end{equation}
\end{theorem}
\begin{proof}
  ...
  \begin{equation}\label{sem_calculation_current}
    ...
    \end{aligned}
  \end{equation}
  The key to proving the identity \eqref{sem_definition} is to express the integrals with contributions from $\xi^m_{,n}$ and $\xi^m_{,np}$ as volume integrals using Gauss's theorem and then to repeatedly simplify the integrand by employing the diffeomorphism equivariance equations \eqref{equivariance_eqns} and a variant of integration by parts, in analogy to the operations performed in Ref.~\cite{goma} for first-order theories.

  For the term with two derivatives of $\xi$, this is as follows:
  \begin{equation}\label{sem_calculation_2nd_deriv}
    \begin{multlined}
      ...
      \begin{aligned}
          ...
          &{} = \int_V \big\{ D_j \big\lbrack L_{:A}^{\hphantom{:A}I} J_I^{jp} \gmc{A}{B}{n}{m}\sigma^B\big\rbrack \xi^m_{,n} \omega_p - D_p D_j \big\lbrack L_{:A}^{\hphantom{:A}I} J_I^{jp} \gmc{A}{B}{n}{m}\sigma^B\big\rbrack \xi^m \omega_n\big\}
      \end{aligned}
    \end{multlined}
  \end{equation}
  ...
  \begin{equation}\label{sem_calculation_1st_deriv}
    ...
    \end{multlined}
  \end{equation}
  Putting together \eqref{sem_calculation_current}--\eqref{sem_calculation_2nd_deriv} finally gives
  \begin{equation}
    \begin{aligned}
  ...
  \begin{equation}
    \boldsymbol{\mathcal{T}}^{\,n}_{\,m} = -\frac{\delta L}{\delta u^A} \gmc{A}{B}{n}{m} \sigma^B,
  \end{equation}
