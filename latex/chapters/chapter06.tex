...
As far as massless modes of radiation are concerned, geodesic deviation in \emph{area metric} gravity looks roughly the same: from Eq.~\eqref{area_metric_linearized_poly}, we know that the spatial part $\varphi^{\alpha\beta}$ of the effective metric that constitutes the linearised principal polynomial and enters the deviation equation \eqref{geodesic_deviation} amounts to
\begin{equation}
